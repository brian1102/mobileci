...
- (instancetype)init {
    return [self initWithApplicationKey:[ReactNativeConfig envFor:@"CHAT_APP_ID"]];
}
...
- (NSString *)getApplicationKey {
    ...
    NSLog(@"CHAT_APPLICATION_KEY :: %@", appKey);
    return appKey ? appKey : [ReactNativeConfig envFor:@"CHAT_APP_ID"];
}

- (NSString *)getApplogizModuleName {
    NSString *appModule = [ReactNativeConfig envFor:@"CHAT_MODULE"];
    NSLog(@"CHAT_APPLICATION_MODULE :: %@", appModule);
    return appModule;
}
...
- (void)registerUserWithCompletion:(ALUser *)alUser withHandler:(void (^)(ALRegistrationResponse *rResponse, NSError *error))completion {
    ...
    [alUser setAppModuleName:[self getApplogizModuleName]];
...
- (void)ALDefaultChatViewSettings {
    ...
    // BRAND_COLOR
    [ALApplozicSettings setStatusBarBGColor: [Theme getBrandColor]];
    [ALApplozicSettings setStatusBarStyle:UIStatusBarStyleLightContent];

    // BRAND_COLOR
    [ALApplozicSettings setColorForNavigation:[Theme getBrandColor]];
