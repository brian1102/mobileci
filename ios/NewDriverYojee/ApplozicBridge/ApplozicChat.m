...
RCT_EXPORT_METHOD(channelUnreadCount:
  ...
  ALChannel* channel = [self.channelService getChannelByKey:channelKey];
  if (channel != nil) {
    NSNumber *count = [channel unreadCount];
...
RCT_EXPORT_METHOD(latestMessageForChannel:
                  (NSDictionary *) config
                  successCallback:
                  (RCTResponseSenderBlock) successCallback
                  cancelCallback:
  ...
  ALMessage* latestMessage = [self.messageDBService getLatestMessageForChannel:channelKey excludeChannelOperations:YES];
  
  if (latestMessage) {
    NSDictionary *jsonDic = @{
                              @"message": latestMessage.message,
                              @"createdAtTime": latestMessage.createdAtTime
                              };
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:jsonDic options:NSJSONWritingPrettyPrinted error:nil];
    NSString *jsonString = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
    return successCallback(@[jsonString]);
  }
...
RCT_EXPORT_METHOD(getChannelByClientGroupId:
    ...
    if (!channel) {
      ...
      NSDictionary *jsonDic = @{
                                ...
                                @"clientChannelKey": channel.clientChannelKey,
                                @"name": channel.name
                                };
      NSData *jsonData = [NSJSONSerialization dataWithJSONObject:jsonDic options:NSJSONWritingPrettyPrinted error:nil];
      NSString *jsonString = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
...
RCT_EXPORT_METHOD(login:
    ...
    [self.user setApplicationId:[ReactNativeConfig envFor:@"CHAT_APP_ID"]];
