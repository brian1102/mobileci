...
-(void) keyBoardWillShow:(NSNotification *) notification
{
    ...
    self.checkBottomConstraint.constant = self.view.frame.size.height - keyboardEndFrame.origin.y + navigationWidth + 15;
...
-(void)subProcessSetHeightOfTextViewDynamically
{
    ...
    int newHeight = (self.typingMessageView.frame.size.height-self.sendMessageTextView.frame.size.height) + sizeThatFitsTextView.height + paddingForTextMessageViewHeight;
    if (newHeight < 55) newHeight = 55;
    self.textMessageViewHeightConstaint.constant = newHeight;
}
